...
## 2024-05-26 - Avoid string allocation in sort loop
**Learning:** Joining strings (like `.join(", ")`) inside a `sort_by` closure is extremely expensive because it allocates multiple new strings (O(N log N) allocations) during comparison operations.
**Action:** Compare collections like `Vec<String>` directly using lexicographical comparison (e.g., `left.cmp(&right)`) to avoid these unnecessary allocations during sorting.
