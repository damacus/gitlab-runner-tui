                ...
                match command {
                    Command::Workers => {
                        self.manager_rows = runners
                            .into_iter()
                            .flat_map(|mut r| {
                                let tags_string = std::mem::take(&mut r.tags_string);
                                r.managers.into_iter().map(move |m| ManagerRow {
                                    runner_id: r.id,
                                    runner_tags_string: tags_string.clone(),
                                    manager: m,
                                })
                            })
                            .collect();
...
mod tests {
    ...
    fn test_manager_row_creation() {
        ...
        let row = ManagerRow {
            runner_id: 12345,
            runner_tags_string: "alm, prod".to_string(),
            manager: manager.clone(),
        };

        assert_eq!(row.runner_id, 12345);
        assert_eq!(row.runner_tags_string, "alm, prod");
